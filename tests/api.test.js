...
async function main() {
  ...
  try {
    ...
    const ui = await fetch('http://127.0.0.1:33333/');
    assert.strictEqual(ui.status, 200);
    assert.ok((await ui.text()).includes('온라인 학습'));
    ...
    const answer = await client.answer('하이');
    assert.strictEqual(answer.category, 'general');
    assert.strictEqual(Object.hasOwn(answer, 'sources'), false);

    await client.teach('api학습테스트키', 'api 학습 답변이 저장됐어.', { category: 'general' });
    ...
    const chat = await client.chatCompletions([
      { role: 'user', content: 'Udon_M1 뭐야' }
    ]);
    assert.strictEqual(chat.object, 'chat.completion');
    assert.ok(
      chat.choices[0].message.content.includes('Udon_M1') ||
      chat.choices[0].message.content.includes('테스트') ||
      chat.choices[0].message.content.includes('우돈')
    );
